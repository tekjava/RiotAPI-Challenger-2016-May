...
<?php
    ...
    if(isset($_GET['enable'])) {
        ...
        $sql = "SELECT sum_id FROM players_data_EUNE";
        ...
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $leagueData = json_decode(file_get_contents('https://eune.api.pvp.net/api/lol/eune/v2.5/league/by-summoner/'.$row['sum_id'].'?api_key='.$apiKey));
            ...
            foreach ($entries as $entry) {
                if(storeData($conn, $apiKey, $entry, 'players_data_EUNE')) {
