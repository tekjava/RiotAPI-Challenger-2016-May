<?php
	...
	if(isset($_GET['userName'])) {
		...
		$sumData = getSummonerInfoDB($conn, $formatedUserName, 'players_data_OCSA');

		if($sumData == false) {
			$sumData = json_decode(file_get_contents("https://oce.api.pvp.net/api/lol/oce/v1.4/summoner/by-name/".$formatedUserName."?api_key=".$apiKey))->$formatedUserName;
		}

		$sumiconid = $sumData->profileIconId;
		$sumId = $sumData->id;
		$url = 'https://oce.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/OC1/'.$sumId.'?api_key='.$apiKey;
		...
		if ($dataHeader[0] == 'HTTP/1.1 200 OK') {
		    ...
		    foreach ($data->participants as $participant) {
				...
				//  echo $name;
				$sumInfoDB = getSummonerInfoDB($conn, $name, 'players_data_OCSA');
			    $champsInfoDB = getChampsInfoDB($conn, $name, 'players_data_OCSA');
			    $playingChamp = NULL;
			    $rankedInfoDB = getTierInfoDB($conn, $name, 'players_data_OCSA');
				$rankedChampsInfoDB = getRankedChampsInfoDB($conn, $name, 'players_data_OCSA');
				//  print_r($champsInfoDB);
				//  echo '<br>';

		    if ($champsInfoDB == false || $champsInfoDB == NULL) {
		    	storeData($conn, $apiKey, $name, 'players_data_OCSA');
				header("Refresh:0");
				...
				if($playingChamp == NULL) {
					$playingChamp = json_decode(file_get_contents('https://oce.api.pvp.net/championmastery/location/OC1/player/'.$sumId.'/champion/'.$participant->championId.'?api_key='.$apiKey));
...
?>
		                                            ...
		                                            <div style="margin-top:-50px;">
														...
														foreach ($playersPlaying['100'] as $blueplayer) {
															$sumSpell1Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/oce/v1.2/summoner-spell/'.$blueplayer['participant']->spell1Id.'?api_key='.$apiKey))->key;
															$sumSpell2Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/oce/v1.2/summoner-spell/'.$blueplayer['participant']->spell2Id.'?api_key='.$apiKey))->key;
