<?php
	...
	if(isset($_GET['userName'])) {
		...
		$sumData = getSummonerInfoDB($conn, $formatedUserName, 'players_data_BR');

		if($sumData == false) {
			$sumData = json_decode(file_get_contents("https://br.api.pvp.net/api/lol/br/v1.4/summoner/by-name/".$formatedUserName."?api_key=".$apiKey))->$formatedUserName;
		}

		$sumiconid = $sumData->profileIconId;
		$sumId = $sumData->id;
		$url = 'https://br.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/BR1/'.$sumId.'?api_key='.$apiKey;
		...
		if ($dataHeader[0] == 'HTTP/1.1 200 OK') {
		    ...
		    foreach ($data->participants as $participant) {
				...
				//  echo $name;
				$sumInfoDB = getSummonerInfoDB($conn, $name, 'players_data_BR');
			    $champsInfoDB = getChampsInfoDB($conn, $name, 'players_data_BR');
			    $playingChamp = NULL;
			    $rankedInfoDB = getTierInfoDB($conn, $name, 'players_data_BR');
				$rankedChampsInfoDB = getRankedChampsInfoDB($conn, $name, 'players_data_BR');
		    ...
		    if ($champsInfoDB == false || $champsInfoDB == NULL) {
				...
				if($playingChamp == NULL) {
					$playingChamp = json_decode(file_get_contents('https://br.api.pvp.net/championmastery/location/BR1/player/'.$sumId.'/champion/'.$participant->championId.'?api_key='.$apiKey));
    ...
    <div id="wrapOuter">
        ...
        <nav>
            ...
            <div class="HeaderTools">
		        <dl>
			    <dd class="Region">
			        <a href="#" onclick="showIFrame('#');return false;"><i class='__spSite __spSite-105'></i><?php echo $lang['LANG_TOP']; ?></a>
			    </dd>
			    </dl>
		    </div>
			<!-- Language Bar CLOSE -->
...
?>
		                                            ...
		                                            <div style="margin-top:-50px;" id='blue'>
														...
														foreach ($playersPlaying['100'] as $blueplayer) {
															$sumSpell1Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/br/v1.2/summoner-spell/'.$blueplayer['participant']->spell1Id.'?api_key='.$apiKey))->key;
															$sumSpell2Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/br/v1.2/summoner-spell/'.$blueplayer['participant']->spell2Id.'?api_key='.$apiKey))->key;
