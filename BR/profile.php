<?php
	...
	if(isset($_GET['userName'])) {
		...
		$sql = 'SELECT * FROM players_data_BR WHERE sum_name = "' . $formatedUserName.'"';
		...
		if($result->num_rows > 0) {
			...
			storeData($conn, $apiKey, $_GET['userName'], 'players_data_BR');
			header("Refresh:0");
		}
	} else {
		header("Location: /");
...
<body> <!-- BODY OPENS -->
    ...
    <div id="dataProvider" data-current-realm="BR" data-current-name="<?php print $summonerName; ?>"></div> <!-- Data Provider -->
    <div id="wrapOuter">
									        ...
									        <div class="LadderRank">
												...
												<?php
                                                    if (isset($_GET['userName'])) {
                                                        ...
                                                        if(checkLimitTime($conn, $_GET['userName'], 'players_data_BR') < 3600) {
														    ...
														    <div class="Summary">
															    <div class="WinRatioTitle">
																    ...
																    &nbsp; | &nbsp; <?php echo $lang['BOX_TOTAL_MASTERY_CP']; ?>
																		...
																		<?php
																			$getRank = "SELECT position FROM top_list_BR WHERE sum_name='".mb_strtolower(str_replace(' ', '', $summonerName), 'UTF-8')."' AND champ_id=".$maxCPChamp;
