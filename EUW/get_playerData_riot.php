...
class PlayerData {
    ...
    public function getSummonerTier() {
        ...
        return json_decode(file_get_contents('https://euw.api.pvp.net/api/lol/euw/v2.5/league/by-summoner/' . $sumonerId . '/entry?api_key=' . $this->apiKey))->$sumonerId;
    }

    public function getRankedChampsInfo() {
        sleep(2);
        return json_decode(file_get_contents('https://euw.api.pvp.net/api/lol/euw/v1.3/stats/by-summoner/' . $this->summonerData->id . '/ranked?season=SEASON2016&api_key=' . $this->apiKey));
    }
}
