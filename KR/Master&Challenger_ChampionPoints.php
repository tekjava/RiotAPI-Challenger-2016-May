...
<div id="dataProvider" data-static-path="# site main pg" data-current-realm="KR" data-current-name=""></div>
<div id="wrapOuter">

<nav>
<div class="HeaderTools"><dl><dd class="Region"><a href="#" onclick="showIFrame('#');return false;"><i class='__spSite __spSite-109'></i><?php echo $lang['LANG_TOP']; ?></a></dd></dl></div>
...
<div class="ContentWrap">
    ...
    $sql = "SELECT sum_name, summoner_info, tier_info FROM challengerandmaster_data_KR";
    ...
    if($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            array_push($players,
                array('name' => $summonerData->name, 'profileIconId' => $summonerData->profileIconId, 'tier' => $tierData->tier, 'division' => $tierEntry->division, 'lp' => $tierEntry->leaguePoints, 'time' => $epoch->format('d-m-y H:i:s'), 'championPoints' => getChampPoints($conn, $row['sum_name'], "challengerandmaster_data_KR"))
            );
            array_push($playersOrder, getChampPoints($conn, $row['sum_name'], "challengerandmaster_data_KR"));
        }
