...
<?php
    ...
    if(isset($_GET['enable'])) {
        ...
        $master_elo = file_get_contents("https://na.api.pvp.net/api/lol/na/v2.5/league/master?type=RANKED_SOLO_5x5&api_key=".$apiKey);
        ...
        for ($i=0; $i < count($master_elo_players); $i++) {
            ...
            if(storeData($conn, $apiKey, (int) $master_elo_players[$i]->playerOrTeamId, "challengerandmaster_data_NA")) {
        ...
        // Get all of the player ids in master league
        $challenger_elo = file_get_contents("https://na.api.pvp.net/api/lol/na/v2.5/league/challenger?type=RANKED_SOLO_5x5&api_key=".$apiKey);
        ...
        for ($i=0; $i < count($challenger_elo_players); $i++) {
            ...
            if(storeData($conn, $apiKey, (int) $challenger_elo_players[$i]->playerOrTeamId, "challengerandmaster_data_NA")) {
                echo "Stored: ".$challenger_elo_players[$i]->playerOrTeamId;
            }
            echo "<br>";
        }

        $sql = "SELECT sum_id FROM challengerandmaster_data_NA";
        ...
        foreach ($playersDelete as $playerDelete) {
            $sql = "DELETE FROM challengerandmaster_data_NA WHERE sum_id=".$playerDelete;
            if($conn->query($sql)) {
