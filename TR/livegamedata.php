<?php
	...
	if(isset($_GET['userName'])) {
		...
		$sumData = getSummonerInfoDB($conn, $formatedUserName, 'players_data_TR');

		if($sumData == false) {
			$sumData = json_decode(file_get_contents("https://tr.api.pvp.net/api/lol/tr/v1.4/summoner/by-name/".$formatedUserName."?api_key=".$apiKey))->$formatedUserName;
		}

		$sumiconid = $sumData->profileIconId;
		$sumId = $sumData->id;
		$url = 'https://tr.api.pvp.net/observer-mode/rest/consumer/getSpectatorGameInfo/TR1/'.$sumId.'?api_key='.$apiKey;
		...
		if ($dataHeader[0] == 'HTTP/1.1 200 OK') {
		    ...
		    foreach ($data->participants as $participant) {
				...
				//  echo $name;
				$sumInfoDB = getSummonerInfoDB($conn, $name, 'players_data_TR');
			    $champsInfoDB = getChampsInfoDB($conn, $name, 'players_data_TR');
			    $playingChamp = NULL;
			    $rankedInfoDB = getTierInfoDB($conn, $name, 'players_data_TR');
				$rankedChampsInfoDB = getRankedChampsInfoDB($conn, $name, 'players_data_TR');
				//  print_r($champsInfoDB);
				//  echo '<br>';

		    if ($champsInfoDB == false || $champsInfoDB == NULL) {
		    	storeData($conn, $apiKey, $name, 'players_data_TR');
				header("Refresh:0");
				...
				if($playingChamp == NULL) {
					$playingChamp = json_decode(file_get_contents('https://tr.api.pvp.net/championmastery/location/TR1/player/'.$sumId.'/champion/'.$participant->championId.'?api_key='.$apiKey));
    ...
    <div id="wrapOuter">
        ...
        <nav>
            ...
            <div class="HeaderTools">
		        <dl>
			    <dd class="Region">
			        <a href="#" onclick="showIFrame('#');return false;"><i class='__spSite __spSite-115'></i><?php echo $lang['LANG_TOP']; ?></a>
			    </dd>
			    </dl>
		    </div>
			<!-- Language Bar CLOSE -->
...
?>
		                                            ...
		                                            <div style="margin-top:-50px;" id="blue">
														...
														foreach ($playersPlaying['100'] as $blueplayer) {
															$sumSpell1Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/tr/v1.2/summoner-spell/'.$blueplayer['participant']->spell1Id.'?api_key='.$apiKey))->key;
															$sumSpell2Name = json_decode(file_get_contents('https://global.api.pvp.net/api/lol/static-data/tr/v1.2/summoner-spell/'.$blueplayer['participant']->spell2Id.'?api_key='.$apiKey))->key;
