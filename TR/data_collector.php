...
<?php
    ...
    if(isset($_GET['enable'])) {
        ...
        $sql = "SELECT sum_id FROM players_data_TR";
        ...
        if($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                $leagueData = json_decode(file_get_contents('https://tr.api.pvp.net/api/lol/tr/v2.5/league/by-summoner/'.$row['sum_id'].'?api_key='.$apiKey));
            ...
            foreach ($entries as $entry) {
                if(storeData($conn, $apiKey, $entry, 'players_data_TR')) {
