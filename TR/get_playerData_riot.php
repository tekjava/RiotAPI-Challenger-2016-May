...
class PlayerData {
    ...
    function __construct($apiKey, $summonerNameOrId) {
      sleep(1);
        ...
        if(gettype($summonerNameOrId) == 'integer') {
            $url = 'https://tr.api.pvp.net/api/lol/tr/v1.4/summoner/' . $summonerNameOrId . '?api_key=' . $apiKey;
            $resultHeaders = get_headers($url);
            //echo 'Found int!';
        } else {
        	$url = 'https://tr.api.pvp.net/api/lol/tr/v1.4/summoner/by-name/' . $summonerNameOrId . '?api_key=' . $apiKey;
            $resultHeaders = get_headers($url);
            // echo 'Found name!';
        }
    ...
    public function getChampsInfo() {
      sleep(2);
        return json_decode(file_get_contents('https://tr.api.pvp.net/championmastery/location/TR1/player/' . $this->summonerData->id . '/champions?api_key=' . $this->apiKey));
    }
    ...
    public function getSummonerTier() {
        ...
        return json_decode(file_get_contents('https://tr.api.pvp.net/api/lol/tr/v2.5/league/by-summoner/' . $sumonerId . '/entry?api_key=' . $this->apiKey))->$sumonerId;
    }

    public function getRankedChampsInfo() {
        sleep(2);
        return json_decode(file_get_contents('https://tr.api.pvp.net/api/lol/tr/v1.3/stats/by-summoner/' . $this->summonerData->id . '/ranked?season=SEASON2016&api_key=' . $this->apiKey));
    }
}
