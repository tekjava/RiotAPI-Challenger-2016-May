...
class PlayerData {
    ...
    function __construct($apiKey, $summonerNameOrId) {
      sleep(1);
        ...
        if(gettype($summonerNameOrId) == 'integer') {
            $url = 'https://las.api.pvp.net/api/lol/las/v1.4/summoner/' . $summonerNameOrId . '?api_key=' . $apiKey;
            $resultHeaders = get_headers($url);
            //echo 'Found int!';
        } else {
        	$url = 'https://las.api.pvp.net/api/lol/las/v1.4/summoner/by-name/' . $summonerNameOrId . '?api_key=' . $apiKey;
            $resultHeaders = get_headers($url);
            // echo 'Found name!';
        }
    ...
    public function getChampsInfo() {
      sleep(2);
        return json_decode(file_get_contents('https://las.api.pvp.net/championmastery/location/LA2/player/' . $this->summonerData->id . '/champions?api_key=' . $this->apiKey));
    }
