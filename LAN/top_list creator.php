...
$emptyQuery = "TRUNCATE top_list_LAN;";
if($conn->query($emptyQuery)) {

    $sql = "SELECT sum_name, summoner_info, tier_info, champs_info FROM players_data_LAN";
    ...
    $champsPoints = array();
        ...
        foreach ($playersByCP as $position => $playerByCP) {
                ...
                $sql2 = 'INSERT INTO top_list_LAN (position, table_row, sum_name, champ_id) VALUES ('.$position.',  \''.$tableRow.'\', \''.mb_strtolower(str_replace(' ', '', $playerByCP['playerName']), 'UTF-8').'\', \''.mb_strtolower(str_replace(' ', '', $playerByCP['champId']), 'UTF-8').'\')';
