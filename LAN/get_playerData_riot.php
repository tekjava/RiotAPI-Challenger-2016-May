...
class PlayerData {
    ...
    public function getSummonerTier() {
        ...
        $response = json_decode(file_get_contents('https://lan.api.pvp.net/api/lol/lan/v2.5/league/by-summoner/' . $sumonerId . '/entry?api_key=' . $this->apiKey));
    ...
    public function getRankedChampsInfo() {
        sleep(0.1); // Prevents from spamming
        $response = json_decode(file_get_contents('https://lan.api.pvp.net/api/lol/lan/v1.3/stats/by-summoner/' . $this->summonerData->id . '/ranked?season=SEASON2016&api_key=' . $this->apiKey));
        if(isset($response->status) && $response->status->status_code == 404) {
            return NULL;
        } else {
            return $response;
